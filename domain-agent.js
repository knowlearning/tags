  ...
  async function applyTagging(partition, tag, target, contributor, value, context, valid_start, valid_end) {
    ...
    for (const [field, date] of Object.entries({ valid_start, valid_end })) {
      if (date === null || (date === undefined && tagging[field] === null)) delete tagging[field]
      else if (date !== undefined) tagging[field] = date
    }
  }
